...
private:
  ...
  std::map<int, ConnectionPtr> conns_;  // 一个TcpServer有多个Connection对象
  std::mutex mutex_conns_;
  NewConnCb new_conn_cb_;      // 回调EchoServer::handleNewConnection()
  CloseConnCb close_conn_cb_;  // 回调EchoServer::handleClose()
  ErrorConnCb error_conn_cb_;  // 回调EchoServer::handleError()
  OnMsgCb on_msg_cb_;          // 回调EchoServer::handleMessage()
  SendComplCb send_compl_cb_;  // 回调EchoServer::handleSendComplete()
  std::function<void(EventLoop*)>
    time_out_cb_;  // 回调EchoServer::handleTimeOut()
};
