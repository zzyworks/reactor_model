...
private:
  ...
  std::function<void()> read_callback_; // fd读事件的回调函数，如果是accept_channel，将回调Acceptor::newConncetion()；如果是client_channel，将回调Channel::onMessage()
  std::function<void()> close_callback_; // 关闭fd的回调函数，将回调Conncetion::closeCallback()
  std::function<void()> error_callback_; // fd发生错误的回调函数，将回调Connection::errorCallback()
};
