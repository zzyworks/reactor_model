...
Connection::Connection(EventLoop* loop, Socket* client_sock)
    : loop_(loop), client_sock_(client_sock) {
  this->init();
}
...
Connection::send(const char* data, size_t n) {
  output_buffer_.appendWithHead(
    data, n);  // 把需要发送的数据保存到Connection的发送缓冲区中
  // 注册写事件
...
void
Connection::setOnMsgCallback(
  std::function<void(Connection*, std::string&)> fn) {
  on_msg_callback_ = fn;
}

void
Connection::setSendCompleteCb(std::function<void(Connection*)> fn) {
  send_compl_cb_ = fn;
}
