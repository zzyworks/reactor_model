...
public:
  ...
  void setOnMsgCallback(std::function<void(Connection*, std::string&)> fn);  
	...
	// 设置fd_发送数据完成的回调函数
	void setSendCompCallback(std::function<void(Connection*)> fn);
...
private:
  ...
  std::function<void(Connection*, std::string&)> on_msg_callback_; // 处理报文的回调函数
	// 发送数据完成后的回调函数，将回调TcpServer::sendComplete()
	std::function<void(Connection*)> send_comp_callback_;
};
