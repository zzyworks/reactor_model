...
Acceptor::newConnection() {
  ...
  new_connection_cb_(client_sock); // 回调TcpServer::newConnection()
}

void 
Acceptor::setNewConnectionCb(std::function<void(Socket*)> fn) {
  new_connection_cb_ = fn;
}

void 
Acceptor::init_(unsigned short int type, const char* ip, uint16_t port) {
