...
ThreadPool::ThreadPool(std::size_t thread_n, std::size_t thread_type)
    : stop_(false), thread_type_(thread_type) {
  this->init(thread_n);
}
...
ThreadPool::init(std::size_t thread_n) {
  for (std::size_t i = 0; i != thread_n; ++i) {
    threads_.emplace_back([this](void) {
      std::cout << "create " << (thread_type_ == 0 ? "IO" : "WORK") << " thread."
                << syscall(SYS_gettid) << "\n";
      while (!stop_) {
        Task task;
        {
        ...
        std::cout << (thread_type_ == 0 ? "IO" : "WORK") << " " 
